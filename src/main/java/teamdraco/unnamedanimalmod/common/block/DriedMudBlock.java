...
public class DriedMudBlock extends Block {
    ...
    private static boolean shouldSolidify(LevelAccessor reader, BlockPos pos, BlockState state) {
        return causesSolidify(state) || isTouchingLiquid(reader, pos);
    }

    private static boolean isTouchingLiquid(LevelAccessor reader, BlockPos pos) {
        boolean flag = false;
