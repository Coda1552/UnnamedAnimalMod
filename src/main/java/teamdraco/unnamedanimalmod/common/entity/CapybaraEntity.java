...
public class CapybaraEntity extends TamableAnimal implements MenuProvider {
    private static final LazyLoadedValue<Set<ItemLike>> TEMPT_ITEMS = new LazyLoadedValue<>(() -> {
    ...
    public void tick() {
        ...
        if (getPassengers().isEmpty()) {
            for (Entity e : level.getEntities(this, getBoundingBox().inflate(0.5))) {
                if (e instanceof PathfinderMob && e.getBbWidth() <= 0.75f && e.getBbHeight() <= 0.75f && !this.isBaby() && ((PathfinderMob) e).getMobType() != MobType.WATER && !isInWater()) {
                    e.startRiding(this);
                }
            }
        }
    ...
    @Nullable
    @Override
    public ChestMenu createMenu(int p_createMenu_1_, Inventory p_createMenu_2_, Player p_createMenu_3_) {
        if (inventory == null) {
            return null;
        }
    ...
    static class WaterPathNavigator extends GroundPathNavigation {
        WaterPathNavigator(CapybaraEntity p_i231565_1_, Level p_i231565_2_) {
        ...
        protected boolean hasValidPathType(BlockPathTypes p_230287_1_) {
            return p_230287_1_ == BlockPathTypes.WATER || super.hasValidPathType(p_230287_1_);
        }
